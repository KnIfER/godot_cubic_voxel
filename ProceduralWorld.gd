...
const voxel_set := preload("res://TiledVoxelSet.tres")
...
#(float, 0.01, 10.0, 0.01)
@export var redistribution := 1.8

#(float, 0.0, 1.0, 0.01)
@export var structure_rate := 0.8

#(float, 0.0, 100.0, 1.0)
@export var biome_factor := 10.0

#(int, 0, 10)
@export var chunk_layers := 3
...
func _process(delta):
	if not marker_path.is_empty():
		var marker : Node3D = get_node(marker_path)
		if is_instance_valid(marker):
			var center_chunk := _world_to_chunk(marker.global_position)
...
#	chunk_node.uv_map = true
	...
	for x in range(chunk_size):
		for z in range(chunk_size):
			...
			var noise := pow(
					remap(noise_1 + noise_2 + noise_4, -1, 1, 0, 1),
					biome_redistribution)
			
			var altitude := int(remap(
					noise,
					0, 1, 0, height))
...
#						var structure : VoxelMeshInstance3D = _structures[structure_id][1]
...
#									structure.get_voxel_id(structure_grid))
	...
	chunk_node.position = chunk * chunk_size
	chunk_node.position -= Vector3(1, 0, 1) * (chunk_size / 2)
	chunk_node.position *= voxel_size
	_chunks[chunk] = chunk_node


func _add_chunk(chunk : Vector3) -> void:
	var chunk_node : Node3D = _chunks.get(chunk, null)
