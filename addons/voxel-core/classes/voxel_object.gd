...
func set_mesh_mode(value : int) -> void:
	mesh_mode = value
...
func set_uv_map(value : bool) -> void:
	uv_map = value
...
func set_static_body(value : bool) -> void:
	static_body = value
...
func get_box_transformed(volume := get_voxels()) -> Dictionary:
	...
	box.position = global_transform * box.position # xform
	box.end = global_transform * box.end # xform
...
var empty_callable = Callable(self, "print_args")
...
func intersect_ray(
	...
	direction : Vector3,
	max_distance := 64,
	stop : Callable = empty_callable) -> Dictionary:
	var hit := {
				   "normal": Vector3(),
			   }
	...
	while t < max_distance:
		...
		if get_voxel_id(grid) > -1 or (is_instance_valid(stop) and stop!=empty_callable and stop.call(hit)):
			valid = true
			break
